    ...
    for (let x = 0; x < boardSize; x++){
        for (let y = 0; y < board; y++){
            if (board[x][y].value !== '💣'){
                ...
                if (x-1 >= 0 && y-1 >= 0 && board[x-1][y-1].value === '💣'){
                    mineNum += 1;
                }
                if (x-1 >= 0 && y+1 < boardSize && board[x-1][y+1].value === '💣'){
                    mineNum += 1;
                }
                if (x+1 < boardSize && y-1 >= 0 && board[x+1][y-1].value === '💣'){
                    mineNum += 1;
                }
                if (x+1 < boardSize && y+1 < boardSize && board[x+1][y+1].value === '💣'){
                    mineNum += 1;
                }
                board[x][y].value = mineNum;
            }
        }
    }
    printBoard();
